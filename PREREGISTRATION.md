...
Provider routing is pinned in `config.glm.yaml`: Baidu FP8 for GLM 5.3, GMICloud FP8 for DeepSeek V3.2, and Anthropic for Opus 4.6 and the possible Haiku 4.5 replacement. Fallback routing is disabled. Requests use temperature 0. GLM uses the endpoint's required low reasoning effort; DeepSeek and Claude use no reasoning. Exact replay depends on cached responses because not every endpoint supports a seed. OpenRouter's per-request maximum-price filter and the local pre-dispatch ledger enforce the captured rates. The estimates exclude retries; uncertain requests are not retried automatically and retain their reservation until reconciled.
