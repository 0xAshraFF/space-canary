...
The locked allocation is encoded in `config.glm.yaml`. Its current planning estimate is $15.03 total, including $6.94 for Opus, within the $25 overall and $8 frontier caps. Opus 4.6 is a specified stronger-model comparator, not a claim about the latest frontier.

Prices are catalog minima from OpenRouter, captured in `docs/model-prices.json`. Provider-specific price, endpoint identity, reasoning settings, input token accounting, and complete call cost bounds must be fixed before live dispatch. The estimates exclude retries and extra calibration rounds.
